...
Collection.prototype.save = function save (doc, options, callback) {
  ...
  if (id) {
    console.log('[mongo] updating', doc);
    this.update({ _id: id }, doc, { upsert: true, safe: errorOptions }, callback);
  } else {
    console.log('[mongo] inserting', doc);
  ...
  if(errorOptions && errorOptions != false) {    
    ...
    this.db.executeCommand(updateCommand, commandOptions, function (err, error) {
      ...
      if (err) {
        ...
        console.log('[update] wrap.');
        callback(self.db.wrap(error[0]));
      } else {
        console.log('[update] err[0].');
        callback(null, error[0].n);
      }      
    });    
  } else {
    ...
    // If no callback just return
    if (!callback) {
      console.log('[update] no callback.');
      return;
    }
    // If error return error
    if (result instanceof Error) {
      console.log('[update] instanceof Error', result);
      return callback(result);
    }
    // Otherwise just return
    console.log('[update] empty callback.');
    return callback();
  }
};
